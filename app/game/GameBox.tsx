...
import {
  C, PROFILE, CANVAS_W, CANVAS_H, LEVEL_W, GROUND_Y,
  GRAVITY, MOVE_SPEED, JUMP_STRENGTH,
} from "./constants";
import { Rect, Coin, Player } from "./types";
import { drawCoin, drawCloudHUD } from "./draw";
import StatusPanel from "./StatusPanel";
import BusinessCard from "./BusinessCard";
import CVGameFrame from "./GameFrame";
...
export default function GameBox() {
  ...
  function holdLeft(down: boolean) { keys.current.left = down; setPressedLeft(down); }
  function holdRight(down: boolean) { keys.current.right = down; setPressedRight(down); }
  function holdJump(down: boolean) { keys.current.jump = down; setPressedJump(down); }
  ...
  function render(ctx: CanvasRenderingContext2D) {
    ...
    ctx.fillStyle = C.groundEdge; ctx.fillRect(0, GROUND_Y, 10 + LEVEL_W, 10);
  ...
  return (
    // Grid: spil til venstre, status til højre (stack på mobil)
    <div className="grid gap-6 md:grid-cols-[auto,1fr] items-start">
      ...
      <div className="flex justify-center mt-8">
        ...
        {/* Mobil controller overlay (kun inde i denne kasse) */}
        <div
          className="md:hidden"
          style={{
            position: "absolute",
        ...
        >
          ...
          >
            <button
              aria-label="Left"
              onPointerDown={(e) => { e.currentTarget.setPointerCapture(e.pointerId); holdLeft(true); }}
              ...
              style={{
                ...
                background: pressedLeft ? "rgba(255,255,255,0.14)" : "rgba(255,255,255,0.08)",
                color: "#e9edf6", fontSize: 20,
              }}
            >←</button>

            <button
              aria-label="Right"
              onPointerDown={(e) => { e.currentTarget.setPointerCapture(e.pointerId); holdRight(true); }}
              ...
              style={{
                width: 56, height: 56, borderRadius: 12,
                border: "1px solid rgba(255,255,255,0.18)",
                background: pressedRight ? "rgba(255,255,255,0.14)" : "rgba(255,255,255,0.08)",
                color: "#e9edf6", fontSize: 20,
              }}
            >→</button>
          </div>
      ...
      <div className="flex flex-col md:flex-row w-full gap-6 ">
        <div className="w-full md:w-600 bg-white/10 border border-white/20 rounded-xl shadow p-4 mb-4 md:mb-0 flex-shrink-0">
